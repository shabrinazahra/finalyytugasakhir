    ...
    <div class="p-6 max-w-5xl">
        ...
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-500">
                Tambahkan akun kader atau petugas kesehatan
            </p>
        </div>
        ...
        <form method="POST" action="{{ route('users.store') }}" class="space-y-5">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="password" name="password" id="password" autocomplete="new-password"
                        class="w-full border rounded-lg px-3 py-2 pr-10 bg-white focus:ring-2 focus:ring-[#1B3C53] focus:outline-none"
            ...
            {{-- Posyandu --}}
            <div id="posyanduField" class="{{ old('role') == 'kader' ? '' : 'hidden' }}">
                <label class="block text-sm font-medium text-gray-700 mb-1">
                    Posyandu
                </label>

                <select name="posyandu_id"
                    class="w-full border rounded-lg px-3 py-2 bg-white focus:ring-2 focus:ring-[#1B3C53]">

                    <option value="">-- Pilih Posyandu --</option>

                    @foreach ($posyandus as $p)
                    @php
                    $isOccupied = $p->kader_count > 0;
                    @endphp
